...
// Variables para la calibración y el cálculo del promedio de los ángulos.
float sum_avg_roll = 0, sum_avg_pitch = 0, avg_roll, avg_pitch, roll_c, pitch_c;
int calib_count = 0;
const int averaging_elements = 100;
...
void loop() {
  ...
  // Calcula los ángulos de inclinación y orientación (roll y pitch) usando los datos del acelerómetro.
  roll_acc = atan2(Accy, Accz) * 180 / M_PI;
  pitch_acc = atan2(-Accx, sqrt(Accy * Accy + Accz * Accz)) * 180 / M_PI;

  // Calcula los ángulos de inclinación y orientación (roll y pitch) usando los datos del giroscopio.
  dt = (micros() - t_ant) / 1000000.0;  // Calcula el intervalo de tiempo transcurrido.
  roll_gyr = roll + dt * Gyrx;  // Integra la velocidad angular para obtener el ángulo.
  pitch_gyr = pitch + dt * Gyry;
  ...
  // Calibración: calcula el promedio de los primeros valores para determinar un offset.
  if (calib_count < averaging_elements) {
    sum_avg_roll += roll;
    sum_avg_pitch += pitch;
    calib_count++;
  } else if (calib_count == averaging_elements) {
    avg_roll = sum_avg_roll / averaging_elements;
    avg_pitch = sum_avg_pitch / averaging_elements;
    calib_count++;
  }

  // Aplica la calibración a los ángulos calculados.
  if (calib_count > averaging_elements) {
    roll_c = roll - avg_roll;
    pitch_c = pitch - avg_pitch;

    // Muestra los resultados por el puerto serial.
    Serial.print("Roll: "); Serial.print(roll_c);
    Serial.print(" Pitch: "); Serial.print(pitch_c);
  ...
  delay(20);  // Espera un corto tiempo antes de la próxima iteración del bucle.
}
